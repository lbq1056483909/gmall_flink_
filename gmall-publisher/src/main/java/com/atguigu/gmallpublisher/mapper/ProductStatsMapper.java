...
public interface ProductStatsMapper {
    //获取某一天商品的交易额
    @Select("select sum(order_amount) order_amount from product_stats_210826 where toYYYYMMDD(stt)=#{date}")
    BigDecimal getGMV(int date);

    /*
    获取某一天不同品牌的交易额
    如果mybatis的方法中，有多个参数，每个参数前需要用@Param注解指定参数的名称*/
    @Select("select tm_id,tm_name,sum(order_amount) order_amount from product_stats_210826 " +
     ...
     * 获取某一天不同品类的交易额
     */
    @Select("select category3_id,category3_name,sum(order_amount) order_amount from product_stats_210826 " +
     ...
     * 获取某一天不同SPU的交易额
     */
    @Select("select spu_id,spu_name,sum(order_amount) order_amount,sum(order_ct) order_ct from product_stats_210826 " +
